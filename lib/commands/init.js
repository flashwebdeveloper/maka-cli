...
Command.create({
  ...
  return CurrentConfig.withValue(config, function () {
    ...
    if (config.engines.test !== 'none') {
      ...
      self.installNpmPackage('jquery', {cwd: appDirectory});
    }
    ...
    if (config.engines.client === 'reflux') {
        self.installNpmPackage('reflux', {cwd: appDirectory});
    }
    ...
    if (config.engines.graphql === 'apollo') {
        ...
        self.installNpmPackage('graphql@0.11.7', {cwd: appDirectory});
        ...
        self.installNpmPackage('react-apollo@beta', {cwd: appDirectory});
        self.installNpmPackage('apollo-boost', {cwd: appDirectory});
    ...
    if (config.engines.ssr === 'true' || config.engines.client === 'react') {
        self.removeMeteorPackage('blaze-html-templates', {cwd: appDirectory});
        self.installMeteorPackage('static-html', {cwd: appDirectory});
    }

    if (config.engines.ssr === 'true' && config.engines.theme === 'material') {
        self.installNpmPackage('jss', {cwd: appDirectory});
        self.installNpmPackage('react-jss', {cwd: appDirectory});
        self.installNpmPackage('jss-preset-default', {cwd: appDirectory});
    }
